#!/usr/bin/env python3
"""
🌟 xAction Enhanced Internal Models - COMBINED Implementation
Internal components and utilities for comprehensive action management.

Features:
- Enhanced ActionRegistry with COMBINED support
- OpenAPI 3.1 export capabilities
- Profile-based action organization
...
if TYPE_CHECKING:
    from .facade import xAction, ActionProfile  # Import xAction for type hints only
...
class EnhancedActionRegistry:
    ...
    # Core storage
    _actions: Dict[str, Dict[str, 'xAction']] = {}
    ...
    @classmethod
    def register(cls, entity_type: str, action: 'xAction'):
    ...
    @classmethod
    def get_actions_for(cls, entity_type: str) -> Dict[str, 'xAction']:
        """Get all actions for an entity type."""
        return cls._actions.get(entity_type, {})
    
    @classmethod
    def get_actions_by_profile(cls, profile: str) -> List['xAction']:
    ...
    @classmethod
    def get_actions_by_tag(cls, tag: str) -> List['xAction']:
    ...
    @classmethod
    def export_profile_summary(cls) -> Dict[str, Any]:
        """Export summary of action profiles usage."""
        return {
            "total_profiles": len(cls._profiles),
            "profiles": dict(cls._profiles),
            "profile_metrics": dict(cls._metrics["profiles"])
        }
    
    @classmethod
    def export_security_summary(cls) -> Dict[str, Any]:
        """Export summary of security configurations."""
        return {
            "total_schemes": len(cls._security_schemes),
            "schemes": list(cls._security_schemes),
            "scheme_usage": dict(cls._metrics["security_schemes"])
        }
    ...
    def find_actions(cls, 
                    ...
                    audit_enabled: Optional[bool] = None) -> List['xAction']:
        """
...
ActionRegistry = EnhancedActionRegistry 